...
def train(args: SaFormerTrainConfig):
    ...
    if args.group is None:
        args.group = args.task + "-biAug-" + str(int(args.cost_limit))
    ...
    rt_inv = lambda x: x*dataset.info['rtg_std'] + dataset.info['rtg_mean']
    ct_inv = lambda x: x*dataset.info['ctg_std'] + dataset.info['ctg_mean']

    cfg.update({'rtg_mean':float(dataset.info['rtg_mean']),'rtg_std':float(dataset.info['rtg_std'])})
    cfg.update({'ctg_mean':float(dataset.info['ctg_mean']),'ctg_std':float(dataset.info['ctg_std'])})
    ...
    for step in trange(args.update_steps, desc="Training"):
        ...
        states, actions, returns, costs_return, time_steps, mask, episode_cost, costs, costs_to_go, rewards_to_go = [
        ...
        if (step + 1) % args.eval_every == 0 or step == args.update_steps - 1:
            ...
            for target_return in args.target_returns:
                ...
                name = "SF_c_" + str(int(cost_return)) + "_r_" + str(int(reward_return))
            ...
            for target_return in args.target_returns:
                ...
                if args.cost_reverse:
                    # critical step, rescale the return!
                    ret, cost, length = trainer.evaluate2(
                        args.eval_episodes, reward_return * args.reward_scale,
                        (args.episode_len - cost_return) * args.cost_scale)
                else:
                    ret, cost, length = trainer.evaluate2(
                ...
                name = "noQc_c_" + str(int(cost_return)) + "_r_" + str(int(reward_return))
                log_cost.update({name: cost})
                log_reward.update({name: ret})
                log_len.update({name: length})
            ...
            for target_return in args.target_returns:
                ...
                if args.cost_reverse:
                    # critical step, rescale the return!
                    ret, cost, length = trainer.evaluate3(
                        args.eval_episodes, reward_return * args.reward_scale,
                        (args.episode_len - cost_return) * args.cost_scale)
                else:
                    ret, cost, length = trainer.evaluate3(
                ...
                name = "CDT_c_" + str(int(cost_return)) + "_r_" + str(int(reward_return))
                log_cost.update({name: cost})
                log_reward.update({name: ret})
                log_len.update({name: length})
