...
class SaFormer(nn.Module):
    ...
    def __init__(
        ...
        if self.stochastic:
            ...
            self.rtg_pred_head = DiagGaussianActor(embedding_dim, 1)
            self.cost_return_pred_head = DiagGaussianActor(embedding_dim, 1)
            #self.cost_return_pred_head = nn.Linear(embedding_dim, 1)
    ...
    def forward(
        ...
        if self.reward_transform is not None:
            returns_to_go = self.reward_transform(returns_to_go.detach())
        ...
        ctg_feat = out[:, self.seq_repeat - 4]
        ...
        cost_return_preds = self.cost_return_pred_head(
            action_feature)

        return action_preds, rtg_preds, cost_preds, state_preds, cost_return_preds


class SaFormerTrainer:
    """
    ...
    def __init__(
            ...
            reward_transform_inv = None,
            cost_transform = None,
            cost_transform_inv = None,
            device="cpu") -> None:
        self.model = model
        self.logger = logger
        self.env = env
        ...
        if reward_transform:
            ...
            self.reward_transform = None
        if reward_transform_inv:
            self.reward_transform_inv = reward_transform_inv
        else:
            self.reward_transform_inv = None
        if cost_transform:
            self.cost_transform = cost_transform
        else:
            self.cost_transform = None
        if cost_transform_inv:
            self.cost_transform_inv = cost_transform_inv
        else:
            self.cost_transform_inv = None

    def train_one_step(self, states, actions, returns, costs_return, time_steps, mask,
                       episode_cost, costs, costs_to_go, rewards_to_go):
        # True value indicates that the corresponding key value will be ignored
        padding_mask = ~mask.to(torch.bool)
        action_preds, rtg_preds, cost_preds, state_preds, cost_return_preds = self.model(
            states=states,
            actions=actions,
        ...
        if self.reward_transform is not None:
            rewards_to_go = self.reward_transform(rewards_to_go)
        
        if self.cost_transform is not None:
            costs_return = self.cost_transform(costs_return)


        if self.stochastic:
            log_likelihood = action_preds.log_prob(actions)[mask > 0].mean()
            rtg_log_likelihood = rtg_preds.log_prob(rewards_to_go.unsqueeze(-1))[mask > 0].mean()
            cost_ret_log_likelihood = cost_return_preds.log_prob(costs_return.unsqueeze(-1))[mask > 0].mean()
            ...
            rtg_loss = -rtg_log_likelihood
            cost_ret_loss = -cost_ret_log_likelihood
            self.logger.store(tab="train",
                              nllact=-log_likelihood.item(),
                              nllrtg=-rtg_log_likelihood.item(),
                              nllcostret = -cost_ret_log_likelihood.item(),
                              ent=entropy.item(),
        ...
        # ctg_loss = F.mse_loss(cost_return_preds,costs_return.unsqueeze(-1).detach(),reduction="none")
        # ctg_loss = (ctg_loss*mask.unsqueeze(-1)).mean()

        loss = act_loss + self.cost_weight * cost_loss + self.state_weight * state_loss  + 0.05*(rtg_loss + cost_ret_loss)
    ...
    def rollout(
        ...
        """
        states = torch.zeros(candidate_num,
                             model.episode_len + 1,
                             model.state_dim,
                             dtype=torch.float,
                             device=self.device)
        actions = torch.zeros(candidate_num,
                              model.episode_len,
                              model.action_dim,
                              dtype=torch.float,
                              device=self.device)
        returns = torch.zeros(candidate_num,
                              model.episode_len + 1,
                              dtype=torch.float,
                              device=self.device)
        costs = torch.zeros(candidate_num,
                            model.episode_len + 1,
                            dtype=torch.float,
                            device=self.device)
        time_steps = torch.arange(model.episode_len,
                                  dtype=torch.long,
                                  device=self.device)
        time_steps = time_steps.view(1, -1).repeat(candidate_num, 1)
        ...
        for step in range(model.episode_len):
            ...
            _,rtgs, _, _, _ = model(s, a, r, c, t, None, epi_cost)

            rtg_mean = rtgs.mean[0,-1].cpu().item()
            rtg_std = rtgs.stddev[0,-1].cpu().item()


            rtgs = rtgs.sample()[:,-1].clamp(rtg_mean-1*rtg_std, rtg_mean+1*rtg_std).cpu().numpy().flatten()
            rtgs = np.sort(rtgs)

            if self.reward_transform_inv is not None:
                rtgs = self.reward_transform_inv(rtgs)

            r[:,-1] =  torch.as_tensor(rtgs, device=self.device)

            acts, _, _, _,_ = model(s, a, r, c, t, None, epi_cost)

            acts = acts.mean[:,-1].clamp(-self.max_action, self.max_action)

            a[:,-1] = acts

            _, _, _, _,cost_rets = model(s, a, r, c, t, None, epi_cost)

            cost_rets_mean = cost_rets.mean[:,-1].cpu().numpy().flatten()
            cost_rets_std = cost_rets.stddev[:,-1].cpu().numpy().flatten()

            if self.cost_transform_inv is not None:
                cost_rets = self.cost_transform_inv(cost_rets_mean+0*cost_rets_std)
            
            acts_mask = np.ceil(cost_rets) < costs[0, step].cpu().item()


            acts_mask[0] = True

            # if acts_mask.sum() != 1 and acts_mask.sum() != 32:
            #     print(acts_mask)
            
            acts = acts.cpu().numpy()
            act = acts[acts_mask][-1]
            rtg = rtgs[acts_mask][-1]
            ...
            returns[:, step] = torch.as_tensor(rtg)
    ...
    def rollout_2stage(
        ...
        for step in range(model.episode_len):
            ...
            if self.reward_transform_inv is not None:
                rtg_mean = self.reward_transform_inv(rtg_mean)
            
            returns[:, step] = torch.as_tensor(rtg_mean, device=self.device)
            r = returns[:, :step + 1][:, -model.seq_len:]  # noqa
    ...
    def evaluate2(self, num_rollouts, target_return, target_cost):
        """
        Evaluates the performance of the model on a number of episodes.
        """
        self.model.eval()
        episode_rets, episode_costs, episode_lens = [], [], []
        for _ in trange(num_rollouts, desc="Evaluating...", leave=False):
            epi_ret, epi_len, epi_cost = self.rollout_2stage(self.model, self.env,
                                                      target_return, target_cost)
            episode_rets.append(epi_ret)
            episode_lens.append(epi_len)
            episode_costs.append(epi_cost)
    ...
    def evaluate3(self, num_rollouts, target_return, target_cost):
        """
        Evaluates the performance of the model on a number of episodes.
        """
        self.model.eval()
        episode_rets, episode_costs, episode_lens = [], [], []
        for _ in trange(num_rollouts, desc="Evaluating...", leave=False):
            epi_ret, epi_len, epi_cost = self.rollout_cdt(self.model, self.env,
                                                      target_return, target_cost)
            episode_rets.append(epi_ret)
            episode_lens.append(epi_len)
            episode_costs.append(epi_cost)
