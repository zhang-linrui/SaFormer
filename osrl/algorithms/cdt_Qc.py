...
class CDT_Qc(nn.Module):
    ...
    def __init__(
        ...
        self.cost_ret_pred_head = nn.Linear(embedding_dim, state_dim)
    ...
    def forward(
        ...
        return action_preds, cost_preds, state_preds, cost_return_preds
...
class CDTQcTrainer:
    ...
    def train_one_step(self, states, actions, returns, costs_return, time_steps, mask,
        ...
        action_preds, cost_preds, state_preds,cost_return_preds = self.model(
            states=states,
            actions=actions,
        ...
        cost_ret_loss = F.mse_loss(cost_return_preds,costs_return.unsqueeze(-1).detach(),reduction="none")
        cost_ret_loss = (cost_ret_loss*mask.unsqueeze(-1)).mean()

        loss = act_loss + self.cost_weight * cost_loss + self.state_weight * state_loss + 0.05*cost_ret_loss
