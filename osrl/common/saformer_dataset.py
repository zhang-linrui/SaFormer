...
def process_sequence_dataset(dataset: dict, cost_reverse: bool = False, reward_scale=1.0, cost_scale=1.0):
    '''
    ...
    for i in trange(dataset["rewards"].shape[0], desc="Processing trajectories"):
        ...
        if dataset["terminals"][i] or dataset["timeouts"][i]:
            ...
            episode_data["rewards_to_go"] = discounted_cumsum(episode_data["rewards"], gamma=1)
            episode_data["cost_returns"] = discounted_cumsum(episode_data["costs"],
                                                             gamma=1)
            episode_data["costs_to_go"] = discounted_cumsum(episode_data["costs"],
                                                             gamma=1)
            
            # FOR　antrun
            # if episode_data["cost_returns"][0] > 80:
            #     data_, episode_step = defaultdict(list), 0
            #     continue
            
            traj.append(episode_data)
            traj_len.append(episode_step)
            rtg.extend((reward_scale*episode_data["returns"]).tolist())
            ctg.extend((cost_scale*episode_data["cost_returns"]).tolist())
    ...
    info = {
        ...
        "rtg_mean": np.array(rtg).mean(),
        "rtg_std": np.array(rtg).std() + 1e-6,
        "ctg_mean": np.array(ctg).mean(),
        "ctg_std": np.array(ctg).std() + 1e-6,
...
#     cost_ret_range = cost_ret.copy()
#     pf_rew_ret = pareto_frontier(cost_ret_range)


#     # import matplotlib.pyplot as plt

#     # ctg_list=np.array(cost_ret_range)
#     # rtg_list=np.array(pf_rew_ret)
...
#     sampled_rew_ret = pf_rew_ret.copy()
...
#     for j,(i, target) in enumerate(zip(nearest_idx, sampled_data)):
#         target_cost_ret, target_rew_ret = target[0], target[1]
#         associated_traj = copy.deepcopy(trajs[i])
#         associated_traj2 = copy.deepcopy(trajs[j])
#         # cost_ret, rew_ret = associated_traj["cost_returns"], associated_traj["returns"]

#         # xx1.append(target_cost_ret)
#         # yy1.append(target_rew_ret)

#         # xx2.append(cost_ret[0])
#         # yy2.append(rew_ret[0])

#         # plt.plot(np.array([target_cost_ret,cost_ret[0]]),np.array([target_rew_ret,rew_ret[0]]))

#         ctg = associated_traj["cost_returns"]
#         ctg += target_cost_ret - ctg[0]

#         rtg = associated_traj2["rewards_to_go"]
#         rtg += associated_traj["rewards_to_go"][0] - rtg[0]

#         aug_trajs.append(associated_traj)
#         aug_trajs.append(associated_traj2)
...
def augmentation(trajs: list,
    ...
    # # the augmented data should be within the cost return range of the dataset
    # cost_ret_range = np.linspace(np.min(cost_ret), np.max(cost_ret), sample_num)
    cost_ret_range = np.unique(cost_ret.copy())
    pf_rew_ret = pareto_frontier(cost_ret_range)



    # sample the rewards that are above the pf curve and within the max_reward
    sampled_cost_ret_range = np.linspace(np.min(cost_ret), np.max(cost_ret), sample_num)
    sampled_pf_rew_ret = pareto_frontier(sampled_cost_ret_range)
    max_reward = max_reward * np.ones(sampled_pf_rew_ret.shape)
    min_reward = min_reward * np.ones(sampled_pf_rew_ret.shape)
    sampled_rew_ret = np.random.uniform(low=sampled_pf_rew_ret + min_reward,
                                        high=max_reward,
                                        size=sample_num)
    ...
    ctg_list=np.array(cost_ret)
    rtg_list=np.array(rew_ret)
    traj_idx = np.arange(len(ctg_list))
    plt.scatter(
        ctg_list[traj_idx],
        rtg_list[traj_idx],
    ...
    ctg_list=np.array(sampled_cost_ret_range)
    rtg_list=np.array(sampled_rew_ret)
    traj_idx = np.arange(len(ctg_list))
    plt.scatter(
        ctg_list[traj_idx],
        rtg_list[traj_idx],
    ...
    pf_data = np.hstack([cost_ret_range[:, None], pf_rew_ret[:, None]]) 
    remained_data = np.hstack([cost_ret[:, None], rew_ret[:, None]])

    remained_pf = defaultdict(int)
    pf_data_idx = np.arange(0, pf_data.shape[0])

    for i in range(remained_data.shape[0]):
        p = remained_data[i, :]
        mask = pf_data[:, 0] <= p[0]
        delta = pf_data[mask, :] - p
        dist = np.hypot(delta[:, 0] , delta[:, 1]*0)
        idx = np.argmin(dist)
        remained_pf[i] = pf_data_idx[mask][idx]


    # for k,v in remained_pf.items():
    #     target_cost_ret, target_rew_ret = pf_data[v]
    #     cost_ret,rew_ret = remained_data[k]
    #     plt.plot(np.array([target_cost_ret,cost_ret]),np.array([target_rew_ret,rew_ret]))

    pf_remained = defaultdict(list)
    pf_remained_prob = defaultdict(list)
    remained_data_idx = np.arange(0, remained_data.shape[0])
    for i in range(pf_data.shape[0]):
        p = pf_data[i, :]
        mask = remained_data[:, 0] <= p[0]
        # mask = np.logical_and(mask,
        #                         remained_data[:, 0] >= p[0]-15)

        # the associated data should be: 1) smaller than the current cost 2) greater than certain reward
        mask = np.logical_and(mask,
                                remained_data[:, 1] >= p[1] - max_rew_decrease)
        delta = remained_data[mask, :] - p
        dist = np.hypot(delta[:, 0] , delta[:, 1])
        idxes = np.argsort(dist)
        pf_remained[i].extend(remained_data_idx[mask][idxes])
        #pf_remained_prob[i].extend(list((1/(1+dist))[idxes]))
        pf_remained_prob[i].extend(list(dist[idxes]))
    
    
    aug_trajs = []
    for i in range(remained_data.shape[0]):

        nearest_pf = remained_pf.get(i)
        associated_candidates = pf_remained.get(nearest_pf)
        selection_prob = pf_remained_prob.get(nearest_pf)
        if i in associated_candidates:
            associated_traj_src = copy.deepcopy(trajs[i])
            # if associated_traj_src["cost_returns"][0] > 60:
            #     continue
            aug_trajs.append(associated_traj_src)
        else:
            try:
                associated_id = np.random.choice(associated_candidates,p=selection_prob/sum(selection_prob))
            except:
                continue
            #associated_id = np.random.choice(associated_candidates,p=selection_prob/sum(selection_prob))
            associated_traj_src = copy.deepcopy(trajs[i])
            associated_traj_dst = copy.deepcopy(trajs[associated_id])

            # if associated_traj_src["cost_returns"][0] > 60:
            #     continue

            plt.plot(np.array([associated_traj_src["cost_returns"][0],associated_traj_dst["cost_returns"][0]]),np.array([associated_traj_src["returns"][0],associated_traj_dst["returns"][0]]))
        
            ctg = associated_traj_dst["costs_to_go"]
            ctg += associated_traj_src["costs_to_go"][0] - ctg[0]

            rtg = associated_traj_src["rewards_to_go"]
            rtg += associated_traj_dst["rewards_to_go"][0] - rtg[0]

            aug_trajs.append(associated_traj_dst)
            aug_trajs.append(associated_traj_src)
    
    # associate each sampled (cost, reward) pair with a trajectory index
    original_data = np.hstack([cost_ret[:, None], rew_ret[:, None]])
    sampled_data = np.hstack([sampled_cost_ret_range[:, None], sampled_rew_ret[:, None]])
    nearest_idx = get_nearest_point(original_data, sampled_data, max_rew_decrease, beta)

    # relabel the dataset
    for i, target in zip(nearest_idx, sampled_data):
        target_ctg, target_rew_ret = target[0], target[1]
        associated_traj = copy.deepcopy(trajs[i])
        # if target_ctg > 60:
        #     continue
        ctg, rew_ret = associated_traj["costs_to_go"], associated_traj["returns"]
        plt.plot((ctg[0],target_ctg),(rew_ret[0],target_rew_ret))
        ctg += target_ctg - ctg[0]
...
class SequenceDataset(IterableDataset):
    ...
    def __init__(
        ...
        self.original_data, self.info = process_sequence_dataset(dataset, cost_reverse, reward_scale, cost_scale)
    ...
    def __prepare_sample(self, traj_idx, start_idx):
        ...
        rewards_to_go = traj["rewards_to_go"][start_idx:start_idx + self.seq_len]
        costs_to_go = traj["costs_to_go"][start_idx:start_idx + self.seq_len]
        ...
        costs_to_go = costs_to_go * self.cost_scale
        ...
        if states.shape[0] < self.seq_len:
            ...
            costs_to_go = pad_along_axis(costs_to_go, pad_to=self.seq_len)
            rewards_to_go = pad_along_axis(costs_to_go, pad_to=self.seq_len)

        return states, actions, returns, cost_returns, time_steps, mask, episode_cost, costs , costs_to_go, rewards_to_go
